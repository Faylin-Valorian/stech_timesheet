...
export const TravelFeature = {
    ...
    async load(params) {
        try {
            const data = await StechAPI.request('get', `/api/analysis/travel?${params}`);
            this.renderStats(data.travel);
            this.initAndRender(data.states, data.counties);
        } catch (err) { 
    ...
    renderStats(travel) {
        document.getElementById('tr-miles').innerText = Math.round(travel.total_miles || 0);
        document.getElementById('tr-perdiem').innerText = travel.per_diem_days || 0;
        document.getElementById('tr-overnight').innerText = travel.overnight_stays || 0;
        document.getElementById('tr-exp').innerText = '$' + parseFloat(travel.total_expenses || 0).toFixed(2);
    },
    ...
    // Helper: Force map resize when tab becomes visible
    resizeMap() {
        if (this.map) {
