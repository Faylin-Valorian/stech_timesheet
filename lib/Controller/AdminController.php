...
class AdminController extends Controller {
    ...
    /**
     * Helper to enforce granular access permissions
     * Throws exception if user lacks specific rule access
     */
    private function requireAccess(string $ruleKey): void {
        if (!$this->analysisService->checkAccess($ruleKey)) {
            throw new \Exception("Access Denied: Missing permission '$ruleKey'");
        }
    }
    ...
    public function getGroups(): DataResponse {
        try { $this->requireAccess('admin_access'); } catch(\Exception $e) { return new DataResponse([], 403); }
    ...
    public function saveAccess(): DataResponse {
        ...
        $this->adminMapper->saveAccessRule($data['rule_key'], json_encode($groups));
        return new DataResponse(['status' => 'success']);
    }

    // =========================================================================
    // PAYROLL & SETTINGS (admin_payroll)
    // =========================================================================
    ...
    public function getSettings(): DataResponse {
        // Allow read if user has payroll access OR global admin access
        if (!$this->analysisService->checkAccess('admin_payroll') && !$this->analysisService->checkAccess('admin_global_access')) {
    ...
    public function saveSetting(): DataResponse {
        try { $this->requireAccess('admin_payroll'); } catch(\Exception $e) { return new DataResponse([], 403); }
    ...
    public function getUsers(): DataResponse { 
        try { $this->requireAccess('admin_users'); } catch(\Exception $e) { return new DataResponse([], 403); }
    ...
    public function getHolidays(): DataResponse { 
        try { $this->requireAccess('admin_holidays'); } catch(\Exception $e) { return new DataResponse([], 403); }
    ...
    public function getJobs(): DataResponse { 
        try { $this->requireAccess('admin_jobs'); } catch(\Exception $e) { return new DataResponse([], 403); }
    ...
    public function getStates(): DataResponse { 
        try { $this->requireAccess('admin_locations'); } catch(\Exception $e) { return new DataResponse([], 403); }
    ...
    public function toggleState(int $id): DataResponse {
        ...
        $qb = $this->db->getQueryBuilder();
        $state = $qb->select('*')->from('stech_states')->where($qb->expr()->eq('id', $qb->createNamedParameter($id)))->executeQuery()->fetch();
    ...
    public function toggleCounty(int $id): DataResponse {
        ...
        $qb->update('stech_counties')->set('is_enabled', $qb->createNamedParameter($newStatus))->where($qb->expr()->eq('id', $qb->createNamedParameter($id)))->execute();
        return new DataResponse(['status' => 'success', 'new_state' => $newStatus]);
    }

    // =========================================================================
    // IMAGES / THUMBNAILS (admin_jobs context for uploads)
    // =========================================================================
    ...
    public function getThumbnail(string $filename) {
        // Read access is open to allow app usage
