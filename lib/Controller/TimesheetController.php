...
class TimesheetController extends Controller {
    ...
    public function __construct(
        ...
        // FIXED: Argument order to match Application.php closure
    ...
    public function getTimesheet(int $id): DataResponse {
        ...
        $ts = $qb->select('*')->from('stech_timesheets')
    ...
    public function saveTimesheet(): DataResponse {
        ...
        $values = [
            'userid' => $uid, 'timesheet_date' => $data['date'], 
            'time_in' => $data['time_in'] ?: null, 'time_out' => $data['time_out'] ?: null,
            'time_break' => (int)$data['break_min'], 'time_total' => (float)$data['total_hours'], 
            ...
            'travel_state' => $data['state'], 'travel_county' => $data['county'],
            'travel_miles' => (int)$data['miles'], 'travel_extra_expenses' => (float)$data['extra_expense'], 
            'archive' => 0
        ];
        ...
        // Restored Activity Sync logic from original controller
        $this->db->getQueryBuilder()->delete('stech_activity')
            ->where($this->db->getQueryBuilder()->expr()->eq('timesheet_id', $this->db->getQueryBuilder()->createNamedParameter($tid)))
            ->executeStatement();
